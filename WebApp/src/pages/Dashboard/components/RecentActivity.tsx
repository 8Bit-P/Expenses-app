...
export default function RecentActivity() {
  const { transactions, loading } = useTransactions({ pageSize: 5 });
    ...
    <div className="bg-surface-container-lowest p-6 rounded-2xl shadow-sm border border-outline-variant/5">
      ...
      <div className="space-y-1">
        {loading ? (
          ...
          <p className="text-center py-8 text-xs font-medium text-on-surface-variant italic">No recent transactions.</p>
        ) : (
          transactions.map((tx) => (
            <div
              key={tx.id}
